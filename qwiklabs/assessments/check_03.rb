# method description
def check_03(handles:, maximum_score:, resources:)
  bigquery = handles['project_0.BigqueryV2']
  ret_hash = { :score => 0, :message => "", :student_message => "" }
  isAvailable = false

  begin
    review_images_object = bigquery.get_table("cymbal".to_s, "review_images".to_s ) || []
    review_videos_object = bigquery.get_table("cymbal".to_s, "review_videos".to_s ) || []
  rescue 
    isAvailable = false
  end 

  if review_images_object && review_videos_object
    isAvailable = true
  end 
